...
pub  fn archive_read_support_format_raw( _a: *mut archive) -> i32 {
    ...
    let info = unsafe {
        &mut *(calloc_safe(1, ::std::mem::size_of::<raw_info>() as u64)
            as *mut raw_info)
    };
...
fn archive_read_format_raw_bid( a: *mut archive_read,  best_bid: i32) -> i32 {
    let archive_read_ahead:*const()=unsafe {
        __archive_read_ahead_safe(a, 1, 0 as *mut ssize_t)
    };
...
fn archive_read_format_raw_read_header(
     a: *mut archive_read,
    ...
    unsafe{ archive_entry_set_pathname_safe(entry, b"data\x00" as *const u8 as *const i8);
...
fn archive_read_format_raw_read_data(
    ...
    *safe_buff =unsafe {
        __archive_read_ahead_safe(a, 1, &mut avail)
    };
    if avail > 0  {
...
pub fn archive_test_archive_read_format_raw_read_data_skip(mut _a: *mut archive) {
    let  a: *mut archive_read = _a as *mut archive_read;
    let  raw_info: *mut raw_info = unsafe { calloc_safe(1, ::std::mem::size_of::<raw_info>() as u64) }
        as *mut raw_info;
    let safe_raw_info : &mut raw_info = unsafe {
        &mut *raw_info
    };
    unsafe {
