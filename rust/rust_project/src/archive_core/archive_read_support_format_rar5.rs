...
unsafe extern "C" fn cdeque_front_fast(mut d: *mut cdeque, mut value: *mut *mut ()) {
    let safe_d = unsafe { &mut *d };
...
unsafe extern "C" fn cdeque_pop_front_fast(mut d: *mut cdeque, mut value: *mut *mut ()) {
    let safe_d = unsafe { &mut *d };
    let safe_value = unsafe { &mut *value };
    unsafe { *safe_value = *(*d).arr.offset(safe_d.beg_pos as isize) as *mut () };
...
unsafe extern "C" fn cdeque_filter_p(mut f: *mut *mut filter_info) -> *mut *mut () {
    return f as size_t as *mut *mut ();
}
/* Convenience function to cast filter_info* to void *. */
unsafe extern "C" fn cdeque_filter(mut f: *mut filter_info) -> *mut () {
...
unsafe extern "C" fn run_arm_filter(mut rar: *mut rar5, mut flt: *mut filter_info) -> i32 {
...
unsafe extern "C" fn apply_filters(mut a: *mut archive_read) -> i32 {
...
unsafe extern "C" fn dist_cache_push(mut rar: *mut rar5, mut value: i32) {
    let safe_rar = unsafe { &mut *rar };
...
unsafe extern "C" fn dist_cache_touch(mut rar: *mut rar5, mut idx: i32) -> i32 {
    let safe_rar = unsafe { &mut *rar };
    let mut q: *mut i32 = safe_rar.cstate.dist_cache.as_mut_ptr();
    let mut i: i32 = 0;
...
unsafe extern "C" fn consume(mut a: *mut archive_read, mut how_many: int64_t) -> i32 {
    let mut ret: i32 = 0;
...
unsafe extern "C" fn skip_bits(mut rar: *mut rar5, mut bits: i32) {
    let safe_rar = unsafe { &mut *rar };
...
unsafe extern "C" fn read_u32(mut a: *mut archive_read, mut pvalue: *mut uint32_t) -> i32 {
    let mut p: *const uint8_t = 0 as *const uint8_t;
    let safe_pvalue = unsafe { &mut *pvalue };
...
unsafe extern "C" fn read_u64(mut a: *mut archive_read, mut pvalue: *mut uint64_t) -> i32 {
    let mut p: *const uint8_t = 0 as *const uint8_t;
    let safe_pvalue = unsafe { &mut *pvalue };
    if read_ahead(a, 8 as i32 as size_t, &mut p) == 0 {
        return 0 as i32;
...
unsafe extern "C" fn bid_standard(mut a: *mut archive_read) -> i32 {
    let mut p: *const uint8_t = 0 as *const uint8_t;
...
unsafe extern "C" fn rar5_bid(mut a: *mut archive_read, mut best_bid: i32) -> i32 {
...
unsafe extern "C" fn skip_unprocessed_bytes(mut a: *mut archive_read) -> i32 {
    let mut rar: *mut rar5 = get_context(a);
    let mut ret: i32 = 0;
    let safe_rar = unsafe { &mut *rar };
...
unsafe extern "C" fn skip_base_block(mut a: *mut archive_read) -> i32 {
    let mut ret: i32 = 0;
    let mut rar: *mut rar5 = get_context(a);
    let safe_rar = unsafe { &mut *rar };
...
unsafe extern "C" fn parse_filter(mut ar: *mut archive_read, mut p: *const uint8_t) -> i32 {
...
unsafe extern "C" fn scan_for_signature(mut a: *mut archive_read) -> i32 {
    let mut p: *const uint8_t = 0 as *const uint8_t;
...
unsafe extern "C" fn advance_multivolume(mut a: *mut archive_read) -> i32 {
    let mut lret: i32 = 0;
    let mut rar: *mut rar5 = get_context(a);
    let safe_rar = unsafe { &mut *rar };
    loop
...
unsafe extern "C" fn process_block(mut a: *mut archive_read) -> i32 {
    let mut p: *const uint8_t = 0 as *const uint8_t;
    let mut rar: *mut rar5 = get_context(a);
    let mut ret: i32 = 0;
    let safe_rar = unsafe { &mut *rar };
 ...
 * */
unsafe extern "C" fn do_uncompress_file(mut a: *mut archive_read) -> i32 {
    let mut rar: *mut rar5 = get_context(a);
    let mut ret: i32 = 0;
...
unsafe extern "C" fn uncompress_file(mut a: *mut archive_read) -> i32 {
    let mut ret: i32 = 0;
    loop {
...
unsafe extern "C" fn verify_checksums(mut a: *mut archive_read) -> i32 {
    let mut verify_crc: i32 = 0;
    let mut rar: *mut rar5 = get_context(a);
    let safe_rar = unsafe { &mut *rar };
...
unsafe extern "C" fn verify_global_checksums(mut a: *mut archive_read) -> i32 {
 ...
 */
unsafe extern "C" fn rar5_signature(mut buf: *mut i8) {
    let mut i: size_t = 0;
    i = 0 as i32 as size_t;
...
unsafe extern "C" fn rar5_read_data_skip(mut a: *mut archive_read) -> i32 {
    let mut rar: *mut rar5 = get_context(a);
    let safe_rar = unsafe { &mut *rar };
    if safe_rar.main.solid() != 0 {
...
unsafe extern "C" fn rar5_cleanup(mut a: *mut archive_read) -> i32 {
    let mut rar: *mut rar5 = get_context(a);
    let safe_rar = unsafe { &mut *rar };
    free_safe(safe_rar.cstate.window_buf as *mut ());
    free_safe(safe_rar.cstate.filtered_buf as *mut ());
...
unsafe extern "C" fn rar5_capabilities(mut a: *mut archive_read) -> i32 {
    return 0 as i32;
}

unsafe extern "C" fn rar5_has_encrypted_entries(mut _a: *mut archive_read) -> i32 {
    /* Unsupported for now. */
    return ARCHIVE_RAR5_DEFINED_PARAM.archive_read_format_encryption_unsupported;
}

unsafe extern "C" fn rar5_init(mut rar: *mut rar5) -> i32 {
    let safe_rar = unsafe { &mut *rar };
    memset_safe(
        rar as *mut (),
        0 as i32,
...
#[no_mangle]
pub unsafe extern "C" fn archive_read_support_format_rar5(mut _a: *mut archive) -> i32 {
    ...
    ret = __archive_read_register_format_safe(
        ...
        Some(rar5_bid as unsafe extern "C" fn(_: *mut archive_read, _: i32) -> i32),
        Some(
            rar5_options
        ...
        Some(rar5_read_data_skip as unsafe extern "C" fn(_: *mut archive_read) -> i32),
        Some(
            rar5_seek_data
                as unsafe extern "C" fn(
                    _: *mut archive_read,
        ...
        Some(rar5_cleanup as unsafe extern "C" fn(_: *mut archive_read) -> i32),
        Some(rar5_capabilities as unsafe extern "C" fn(_: *mut archive_read) -> i32),
        Some(
            rar5_has_encrypted_entries as unsafe extern "C" fn(_: *mut archive_read) -> i32,
        ),
    );
    if ret != ARCHIVE_RAR5_DEFINED_PARAM.archive_ok {
        rar5_cleanup(ar);
...
#[no_mangle]
pub unsafe extern "C" fn archive_test_rar5_empty_function(mut _a: *mut archive) -> i32 {
