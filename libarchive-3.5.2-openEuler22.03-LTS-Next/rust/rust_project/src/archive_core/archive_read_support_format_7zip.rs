...
extern "C" fn archive_read_format_7zip_read_header(
    ...
    if !zip_entry.is_null() && ((safe_zip_entry).folderIndex as libc::c_ulong) < (safe_zip).si.ci.numFolders
    {
        folder = unsafe{&mut *(safe_zip)
            .si
            .ci
            .folders
    ...
    if (safe_zip_entry).ssIndex != -(1 as libc::c_int) as uint32_t {
        (safe_zip).entry_bytes_remaining = unsafe{*(safe_zip)
            .si
            .ss
            .unpackSizes
    ...
    if (safe_zip_entry).mode & 0o170000 as libc::c_int as mode_t == 0o120000 as libc::c_int as mode_t {
...
extern "C" fn archive_read_format_7zip_read_data(
    ...
    *buff = 0 as *const libc::c_void;
    ...
    /* Update checksum */
    if unsafe{(*safe_zip.entry).flg & ((1 as libc::c_int) << 3 as libc::c_int) as libc::c_uint != 0
        && bytes != 0}
    {
...
extern "C" fn archive_read_format_7zip_read_data_skip(
    mut a: *mut archive_read,
) -> libc::c_int {
    ...
    match ret {
        ...
        9 => {
            archive_set_error(
                &mut safe_a.archive as *mut archive,
                -(1 as libc::c_int),
                b"Lzma library error: Corrupted input data\x00" as *const u8 as *const libc::c_char,
            );
        }
        10 => {
            archive_set_error(
...
extern "C" fn init_decompression(
    ...
    match (safe_zip).codec {
                ...
                #[cfg(HAVE_LZMA_H)] _ =>{
                    {
                        ...
                        if !coder2.is_null() {
                            ...
                            match (safe_zip).codec2 {
                                ...
                                3 => {
                                    if (safe_coder2).propertiesSize != 1 as libc::c_int as libc::c_ulong {
                                        unsafe{
                                        archive_set_error(
                                            &mut (safe_a).archive as *mut archive,
                                            -(1 as libc::c_int),
                                            b"Invalid Delta parameter\x00" as *const u8 as *const libc::c_char,
                                        )};
                                        return -(25 as libc::c_int);
                                    }
                                    filters[fi as usize].id = 0x3 as libc::c_ulong;
                                    memset_safe(
                                        &mut delta_opt as *mut lzma_options_delta as *mut libc::c_void,
                                        0 as libc::c_int,
                                        ::std::mem::size_of::<lzma_options_delta>() as libc::c_ulong,
                                    );
                                    delta_opt.type_0 = LZMA_DELTA_TYPE_BYTE;
                                    delta_opt.dist = unsafe{(*(safe_coder2).properties.offset(0 as libc::c_int as isize)
                                        as uint32_t)
                        ...
                        filters[fi as usize].options = 0 as *mut libc::c_void;
                        ff = unsafe{&mut *filters.as_mut_ptr().offset(fi as isize) as *mut lzma_filter};
                        ...
                        filters[fi as usize].options = 0 as *mut libc::c_void;
                        r = lzma_raw_decoder_safe(&mut (safe_zip).lzstream, filters.as_mut_ptr()) as libc::c_int;
        ...
        262658 => {
            match() {
                #[cfg(all(HAVE_BZLIB_H, BZ_CONFIG_ERROR))] _ =>{

                    {
                        ...
                        r = BZ2_bzDecompressInit_safe(&mut (safe_zip).bzstream, 0 as libc::c_int, 0 as libc::c_int);
                        ...
                        if r != 0 as libc::c_int {
                            ...
                            match r {
                                -2 => {
                                    detail = b"invalid setup parameter\x00" as *const u8 as *const libc::c_char
                                }
                                -3 => {
                                    err = 12 as libc::c_int;
                                    detail = b"out of memory\x00" as *const u8 as *const libc::c_char
                                }
                                -9 => detail = b"mis-compiled library\x00" as *const u8 as *const libc::c_char,
                                _ => {}
                            }
                            unsafe{
                            archive_set_error(
                ...
                #[cfg(not(all(HAVE_BZLIB_H, BZ_CONFIG_ERROR)))] _ =>{
                    {
                        unsafe{
                        archive_set_error(
                            &mut (safe_a).archive as *mut archive,
        ...
        197633 => {
            ...
            order = unsafe{*(safe_coder1).properties.offset(0 as libc::c_int as isize) as libc::c_uint};
            msize = archive_le32dec(unsafe{&mut *(safe_coder1).properties.offset(1 as libc::c_int as isize)
            ...
            __archive_ppmd7_functions
                .Ppmd7_Construct
                .expect("non-null function pointer")(&mut (*zip).ppmd7_context)};
            r = unsafe{__archive_ppmd7_functions
                .Ppmd7_Alloc
            ...
            __archive_ppmd7_functions
                .Ppmd7_Init
                .expect("non-null function pointer")(&mut (safe_zip).ppmd7_context, order)};
                unsafe{
            __archive_ppmd7_functions
        ...
        116457729 | 116458243 | 116459265 => {
            if !(safe_a).entry.is_null() {
                ...
                archive_entry_set_is_data_encrypted_safe((safe_a).entry, 1 as libc::c_int as libc::c_char);
                (safe_zip).has_encrypted_entries = 1 as libc::c_int
            }
            unsafe{
            archive_set_error(
...
extern "C" fn decompress(
    ...
    match (safe_zip).codec {
        ...
        197633 => {
            ...
            loop {
                ...
                sym = unsafe{__archive_ppmd7_functions
                    .Ppmd7_DecodeSymbol
                    .expect("non-null function pointer")(
    ...
    */
    if safe_zip.codec2 == 0x303011b as libc::c_int as libc::c_ulong {
        ...
        (safe_zip).main_stream_bytes_remaining = ((safe_zip).main_stream_bytes_remaining as libc::c_ulong)
            .wrapping_sub(
                (safe_zip)
...
extern "C" fn free_decompression(
    mut a: *mut archive_read,
    mut zip: *mut _7zip,
) -> libc::c_int {
    let safe_a = unsafe{&mut *a};
    ...
    if (safe_zip).bzstream_valid != 0 {
        if BZ2_bzDecompressEnd_safe(&mut (safe_zip).bzstream) != 0 as libc::c_int {
            ...
            archive_set_error(
                &mut (safe_a).archive as *mut archive,
                -(1 as libc::c_int),
                b"Failed to clean up bzip2 decompressor\x00" as *const u8 as *const libc::c_char,
            )};
            r = -(30 as libc::c_int)
        }
        (safe_zip).bzstream_valid = 0 as libc::c_int
    ...
    if (safe_zip).ppmd7_valid != 0 {
        unsafe{__archive_ppmd7_functions
            .Ppmd7_Free
            .expect("non-null function pointer")(&mut (*zip).ppmd7_context)};
        (safe_zip).ppmd7_valid = 0 as libc::c_int
    }
...
extern "C" fn parse_7zip_uint64(
    ...
    while i < 8 as libc::c_int {
        if avail as libc::c_int & mask as libc::c_int != 0 {
            ...
            unsafe{*val = (*val as libc::c_ulong).wrapping_add(
...
extern "C" fn read_Digests(
    ...
    */
    ...
    if unsafe{*p as libc::c_int == 0 as libc::c_int }{
        ...
        /* All are defined */
        memset_safe((safe_d).defineds as *mut libc::c_void, 1 as libc::c_int, num);
    }
    (safe_d).digests = calloc_safe(num, ::std::mem::size_of::<uint32_t>() as libc::c_ulong) as *mut uint32_t;
...
extern "C" fn free_Folder(mut f: *mut _7z_folder) {
    ...
    if !(safe_f).coders.is_null() {
        ...
        while (i as libc::c_ulong) < (safe_f).numCoders {
            free_safe(unsafe{(*(safe_f).coders.offset(i as isize)).properties as *mut libc::c_void});
            i = i.wrapping_add(1)
        }
                    ...
                    match unsafe{*p as libc::c_int} {
                        0 => {
                            ...
                            loop {
                                ...
                                if unsafe{read_Folder(a, &mut *(safe_ci).folders.offset(i as isize))
                                    < 0 as libc::c_int}
                                {
                        ...
                        1 => {
                            if parse_7zip_uint64(a, &mut (safe_ci).dataStreamIndex) < 0 as libc::c_int {
                                return -(1 as libc::c_int);
                            }
                            if (100000000 as libc::c_ulonglong)
                                ...
                                if unsafe{!(*p as libc::c_int != 0xc as libc::c_int)} {
                                    ...
                                    's_148: loop {
                                        ...
                                        let mut folder: *mut _7z_folder =
                                            unsafe{&mut *(safe_ci).folders.offset(i as isize)
                                                as *mut _7z_folder};
                                        ...
                                        while (j as libc::c_ulong) < (safe_folder).numOutStreams {
                                            if unsafe{parse_7zip_uint64(
                                                a,
                                                &mut *(safe_folder).unPackSize.offset(j as isize),
                                            ) < 0 as libc::c_int}
                                            {
                                                current_block = 14585062455194940643;
                                                break 's_148;
...
extern "C" fn read_SubStreamsInfo(
    ...
    if type_0 == 0xd as libc::c_int {
        ...
        i = 0 as libc::c_int as libc::c_uint;
        while (i as libc::c_ulong) < numFolders {
            if unsafe{parse_7zip_uint64(a, &mut (*f.offset(i as isize)).numUnpackStreams)
                < 0 as libc::c_int}
            {
                return -(1 as libc::c_int);
            }
            if unsafe{(100000000 as libc::c_ulonglong)
            ...
            unpack_streams = unsafe{(unpack_streams as libc::c_ulong)
    ...
    if unpack_streams != 0 {
        ...
        if (safe_ss).unpackSizes.is_null() || (safe_ss).digestsDefined.is_null() || (safe_ss).digests.is_null()
        {
            return -(1 as libc::c_int);
        }
    }
    ...
    while (i as libc::c_ulong) < numFolders {
        ...
        if unsafe{!((*f.offset(i as isize)).numUnpackStreams == 0 as libc::c_int as libc::c_ulong) }{
            ...
            if type_0 == 0x9 as libc::c_int {
                ...
                while unsafe{(pack as libc::c_ulong) < (*f.offset(i as isize)).numUnpackStreams }{
                    ...
                    sum = unsafe{(sum as libc::c_ulong).wrapping_add(*fresh4) as uint64_t as uint64_t};
                    pack = pack.wrapping_add(1)
                }
            }
            let fresh5 = usizes;
            usizes = unsafe{usizes.offset(1)};
            unsafe{*fresh5 = folder_uncompressed_size(&mut *f.offset(i as isize)).wrapping_sub(sum)}
        }
        i = i.wrapping_add(1)
    }
    if type_0 == 0x9 as libc::c_int {
    ...
    i = 0 as libc::c_int as libc::c_uint;
    while (i as libc::c_ulong) < numFolders {
        if unsafe{(*f.offset(i as isize)).numUnpackStreams != 1 as libc::c_int as libc::c_ulong
            || (*f.offset(i as isize)).digest_defined == 0}
        {
            numDigests = unsafe{(numDigests as libc::c_uint)
...
extern "C" fn read_StreamsInfo(
    ...
    if unsafe{*p as libc::c_int == 0x8 as libc::c_int} {
        if read_SubStreamsInfo(a, &mut (safe_si).ss, (safe_si).ci.folders, (safe_si).ci.numFolders)
            < 0 as libc::c_int
        {
            return -(1 as libc::c_int);
        }
            ...
            17 => {
                ...
                while (i as libc::c_ulong) < (*zip).numFiles {
                    ...
                    *fresh10 = np;
                    ...
                    match () {
                        #[cfg_attr(_WIN32, _DEBUG, cfg(not(__CYGWIN__)))] _ => {
                            let ref mut fresh_cfg_1= (*entries.offset(i as isize)).wname;
                            *fresh_cfg_1 = np as *mut wchar_t;
                        }
                        #[cfg_attr(any(not(_WIN32), not(_DEBUG), __CYGWIN__),)] _ => {}
                    }
...
extern "C" fn fileTimeToUtc(
    ...
    if fileTime as libc::c_ulonglong >= 116444736000000000 as libc::c_ulonglong {
        fileTime = (fileTime as libc::c_ulonglong)
            ...
            as uint64_t;
        /* milli seconds base */
        unsafe{*timep = fileTime.wrapping_div(10000000 as libc::c_int as libc::c_ulong) as time_t;
        /* nano seconds base */
        *ns = fileTime.wrapping_rem(10000000 as libc::c_int as libc::c_ulong) as libc::c_long
            * 100 as libc::c_int as libc::c_long}
    } else {
        unsafe{*timep = 0 as libc::c_int as time_t;
...
extern "C" fn decode_encoded_header_info(
    ...
    if unsafe{(safe_zip).header_offset
        < (safe_si)
            .pi
            .pos
        ...
        || ((safe_si)
            .pi
            .pos
            .wrapping_add(*(safe_si).pi.sizes.offset(0 as libc::c_int as isize)) as int64_t)
            < 0 as libc::c_int as libc::c_long
        || *(safe_si).pi.sizes.offset(0 as libc::c_int as isize) == 0 as libc::c_int as libc::c_ulong
...
extern "C" fn slurp_central_directory(
    ...
    next_header_size = archive_le64dec(unsafe{p.offset(20 as libc::c_int as isize) as *const libc::c_void});
    next_header_crc = archive_le32dec(unsafe{p.offset(28 as libc::c_int as isize) as *const libc::c_void});
    ...
    match unsafe{*p.offset(0 as libc::c_int as isize) as libc::c_int} {
        23 => {
            ...
            if r == 0 as libc::c_int {
                if unsafe{(*(safe_zip).si.ci.folders.offset(0 as libc::c_int as isize)).digest_defined != 0} {
                    next_header_crc =
                        unsafe{(*(safe_zip).si.ci.folders.offset(0 as libc::c_int as isize)).digest}
                } else {
...
extern "C" fn get_uncompressed_data(
        ...
        && (safe_zip).codec2 == -(1 as libc::c_int) as libc::c_ulong
    {
        ...
        (safe_zip).uncompressed_buffer_pointer = unsafe{(safe_zip)
...
extern "C" fn extract_pack_stream(mut a: *mut archive_read, mut minimum: size_t) -> ssize_t {
        ...
        && (safe_zip).codec2 == -(1 as libc::c_int) as libc::c_ulong
    {
        ...
        (safe_zip).folder_outbytes_remaining = ((safe_zip).folder_outbytes_remaining as libc::c_ulong)
    ...
    if (safe_zip).uncompressed_buffer.is_null() {
        ...
        (safe_zip).uncompressed_buffer = malloc_safe((safe_zip).uncompressed_buffer_size) as *mut libc::c_uchar;
        if (safe_zip).uncompressed_buffer.is_null() {
            unsafe{
            archive_set_error(
                &mut (safe_a).archive as *mut archive,
        ...
        || (safe_zip).uncompressed_buffer_bytes_remaining < minimum
    {
        ...
        if (safe_zip).uncompressed_buffer_size < minimum {
            ...
            p = realloc_safe((safe_zip).uncompressed_buffer as *mut libc::c_void, new_size);
            if p.is_null() {
                unsafe{
                archive_set_error(
                    &mut (safe_a).archive as *mut archive,
        ...
        if used != 0 {
            memmove_safe(
                ...
                unsafe{(safe_zip).uncompressed_buffer.offset(used as isize) as *const libc::c_void},
                (safe_zip).uncompressed_buffer_bytes_remaining,
            );
        }
    } else {
    ...
    loop {
        ...
        (safe_zip).folder_outbytes_remaining = ((safe_zip).folder_outbytes_remaining as libc::c_ulong)
            ...
            as uint64_t;
        (safe_zip).uncompressed_buffer_bytes_remaining =
            ((safe_zip).uncompressed_buffer_bytes_remaining as libc::c_ulong).wrapping_add(bytes_out)
                as size_t as size_t;
...
extern "C" fn seek_pack(mut a: *mut archive_read) -> libc::c_int {
    ...
    (safe_zip).pack_stream_inbytes_remaining =unsafe{
        *(safe_zip).si.pi.sizes.offset((safe_zip).pack_stream_index as isize)};
    pack_offset = unsafe{*(safe_zip)
        .si
        .pi
        .positions
...
extern "C" fn read_stream(
    ...
    */
    if (safe_zip).pack_stream_remaining == 0 as libc::c_int as libc::c_uint {
         ...
         */
        if unsafe{(safe_zip).folder_index == 0 as libc::c_int as libc::c_uint
            && ((*(safe_zip)
                .si
                .ci
                .folders
                ...
                || (safe_zip).folder_index != (*(safe_zip).entry).folderIndex)}
        {
            ...
            skip_bytes = unsafe{(*(safe_zip).si.ci.folders.offset((safe_zip).folder_index as isize)).skipped_bytes}
        }
        ...
        r = setup_decode_folder(
            a,
            unsafe{&mut *(safe_zip).si.ci.folders.offset((safe_zip).folder_index as isize)},
            0 as libc::c_int,
        ) as ssize_t;
        if r != 0 as libc::c_int as libc::c_long {
            return -(30 as libc::c_int) as ssize_t;
...
extern "C" fn setup_decode_folder(
    ...
    */
    ...
    coder1 = unsafe{&mut *(safe_folder).coders.offset(0 as libc::c_int as isize) as *mut _7z_coder};
    if (safe_folder).numCoders == 2 as libc::c_int as libc::c_ulong {
        coder2 = unsafe{&mut *(safe_folder).coders.offset(1 as libc::c_int as isize) as *mut _7z_coder}
    } else {
    ...
    if found_bcj2 != 0 {
        ...
        if unsafe{(safe_folder).numCoders == 4 as libc::c_int as libc::c_ulong
            ...
            && (safe_zip).pack_stream_remaining == 4 as libc::c_int as libc::c_uint}
        {
            ...
            if unsafe{(*(safe_folder).bindPairs.offset(0 as libc::c_int as isize)).inIndex
            ...
            } else if unsafe{(*fc.offset(0 as libc::c_int as isize)).codec
                == 0 as libc::c_int as libc::c_ulong
                && (*fc.offset(1 as libc::c_int as isize)).codec
                    == 0 as libc::c_int as libc::c_ulong}
            {
                coder1 = unsafe{&mut *(*folder).coders.offset(2 as libc::c_int as isize) as *mut _7z_coder}
            } else if unsafe{(*fc.offset(0 as libc::c_int as isize)).codec
                == 0 as libc::c_int as libc::c_ulong
                && (*fc.offset(2 as libc::c_int as isize)).codec
                    == 0 as libc::c_int as libc::c_ulong}
            {
                coder1 = unsafe{&mut *(*folder).coders.offset(1 as libc::c_int as isize) as *mut _7z_coder}
            } else if unsafe{(*fc.offset(1 as libc::c_int as isize)).codec
                == 0 as libc::c_int as libc::c_ulong
                && (*fc.offset(2 as libc::c_int as isize)).codec
                    == 0 as libc::c_int as libc::c_ulong}
            {
                coder1 = unsafe{&mut *(*folder).coders.offset(0 as libc::c_int as isize) as *mut _7z_coder}
            } else {
                unsafe{
                archive_set_error(
                    &mut (safe_a).archive as *mut archive,
            ...
            (safe_zip).main_stream_bytes_remaining = unsafe{
                *(safe_folder).unPackSize.offset(2 as libc::c_int as isize)}
        } else if unsafe{!coder2.is_null()
        ...
        if (safe_zip).tmp_stream_buff.is_null() {
            ...
            (safe_zip).tmp_stream_buff = malloc_safe((safe_zip).tmp_stream_buff_size) as *mut libc::c_uchar;
            if (safe_zip).tmp_stream_buff.is_null() {
                unsafe{
                archive_set_error(
                    &mut (safe_a).archive as *mut archive,
...
extern "C" fn x86_Convert(
    ...
    loop {
        ...
        let mut limit: *mut uint8_t = unsafe{data
        ...
        if prevPosT > 3 as libc::c_int as libc::c_ulong {
            ...
            if prevMask != 0 as libc::c_int as libc::c_uint {
                let mut b: libc::c_uchar = unsafe{*p.offset(
                    (4 as libc::c_int - kMaskToBitNumber[prevMask as usize] as libc::c_int)
                        as isize,
                )};
                if unsafe{kMaskToAllowedStatus[prevMask as usize] == 0
        ...
        prevPosT = bufferPos;
        if unsafe{*p.offset(4 as libc::c_int as isize) as libc::c_int == 0 as libc::c_int
            || *p.offset(4 as libc::c_int as isize) as libc::c_int == 0xff as libc::c_int}
        {
            let mut src: uint32_t = unsafe{(*p.offset(4 as libc::c_int as isize) as uint32_t)
                << 24 as libc::c_int
            ...
            loop {
                ...
                b_index = unsafe{kMaskToBitNumber[prevMask as usize] as libc::c_int * 8 as libc::c_int};
    ...
    (safe_zip).bcj_ip =
        ((safe_zip).bcj_ip as libc::c_uint).wrapping_add(bufferPos as uint32_t) as uint32_t as uint32_t;
    return bufferPos;
}
extern "C" fn Bcj2_Decode(
    ...
    buf1 = unsafe{safe_zip.sub_stream_buff[0 as libc::c_int as usize]
    ...
    buf2 = unsafe{safe_zip.sub_stream_buff[1 as libc::c_int as usize]
    ...
    buf3 = unsafe{safe_zip.sub_stream_buff[2 as libc::c_int as usize]
    ...
    if safe_zip.bcj_state == 0 as libc::c_int as libc::c_uint {
        ...
        while ii < 5 as libc::c_int {
            ...
            safe_zip.bcj2_code = safe_zip.bcj2_code << 8 as libc::c_int | *fresh16 as libc::c_uint;
            ii += 1
        }
    ...
    loop {
        ...
        if safe_zip.bcj2_code < bound {
            ...
            if safe_zip.bcj2_range < (1 as libc::c_int as uint32_t) << 24 as libc::c_int {
                ...
                let fresh20 = buffer;
                buffer = unsafe{buffer.offset(1)};
                safe_zip.bcj2_code = unsafe{safe_zip.bcj2_code << 8 as libc::c_int | *fresh20 as libc::c_uint}
            }
            ...
            safe_zip.bcj2_range =
                (safe_zip.bcj2_range as libc::c_uint).wrapping_sub(bound) as uint32_t as uint32_t;
            safe_zip.bcj2_code =
                (safe_zip.bcj2_code as libc::c_uint).wrapping_sub(bound) as uint32_t as uint32_t;
            ...
            if safe_zip.bcj2_range < (1 as libc::c_int as uint32_t) << 24 as libc::c_int {
                ...
                safe_zip.bcj2_code = safe_zip.bcj2_code << 8 as libc::c_int | *fresh21 as libc::c_uint
            }
            if b as libc::c_int == 0xe8 as libc::c_int {
                v = buf1;
            ...
            dest = unsafe{((*v.offset(0 as libc::c_int as isize) as uint32_t) << 24 as libc::c_int
             ...
             */
            safe_zip.odd_bcj_size = (4 as libc::c_int as libc::c_uint).wrapping_sub(i) as size_t;
