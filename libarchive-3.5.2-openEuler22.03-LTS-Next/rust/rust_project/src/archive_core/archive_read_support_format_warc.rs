...
#[no_mangle]
pub extern "C" fn archive_read_support_format_warc(mut _a:
                                                              *mut archive)
 -> libc::c_int {
    ...
    let mut magic_test: libc::c_int =
        __archive_check_magic_safe(_a, ARCHIVE_WARC_DEFINED_PARAM.archive_read_magic
                              , ARCHIVE_WARC_DEFINED_PARAM.archive_state_new,
                              b"archive_read_support_format_warc\x00" as
    ...
    w =
        calloc_safe(1 as libc::c_int as libc::c_ulong,
               ::std::mem::size_of::<warc_s>() as libc::c_ulong) as
            *mut warc_s;
    if w.is_null() {
        archive_set_error_safe!(&mut safe_a.archive as *mut archive,
                          ARCHIVE_WARC_DEFINED_PARAM.enomem,
                          b"Can\'t allocate warc data\x00" as *const u8 as
                              *const libc::c_char);
        return ARCHIVE_WARC_DEFINED_PARAM.archive_fatal
    }
    r =
        __archive_read_register_format_safe(a, w as *mut libc::c_void,
                                       b"warc\x00" as *const u8 as
                                           *const libc::c_char,
                                       Some(_warc_bid as
                                                extern "C" fn(_:
                                                                         *mut archive_read,
                                       ...
                                       Some(_warc_skip as
                                                extern "C" fn(_:
                                                                         *mut archive_read)
                                                    -> libc::c_int), None,
                                       Some(_warc_cleanup as
                                                extern "C" fn(_:
                                                                         *mut archive_read)
                                                    -> libc::c_int), None,
                                       None);
    if r != ARCHIVE_WARC_DEFINED_PARAM.archive_ok { free_safe(w as *mut libc::c_void); return r }
...
extern "C" fn _warc_bid(mut a: *mut archive_read,
    ...
    hdr =
        __archive_read_ahead_safe(a, 12 as libc::c_uint as size_t, &mut nrd) as
            *const libc::c_char;
    if hdr.is_null() {
        /* no idea what to do */
...
extern "C" fn _warc_rdhdr(mut a: *mut archive_read,
 ...
 -> libc::c_int {
    ...
    let mut fnam: warc_string_t =
        warc_string_t{len: 0, str_0: 0 as *const libc::c_char,};
	 ...
	 * and reproduce that functionality here */
        ...
        if nrd < 0 as libc::c_int as libc::c_long {
            ...
            archive_set_error_safe!(&mut safe_a.archive as *mut archive,
                              ...
                              b"Bad record header\x00" as *const u8 as
                                  *const libc::c_char);
            return ARCHIVE_WARC_DEFINED_PARAM.archive_fatal
        } else {
            if buf.is_null() {
        ...
        ver =
            _warc_rdver(buf,
                        unsafe {eoh.offset_from(buf) as libc::c_long as
                            size_t});
        /* we currently support WARC 0.12 to 1.0 */
        if ver == 0 as libc::c_uint {
            archive_set_error_safe!(&mut safe_a.archive as *mut archive,
                              ARCHIVE_WARC_DEFINED_PARAM.archive_errno_misc,
                              b"Invalid record version\x00" as *const u8 as
                                  *const libc::c_char);
            return ARCHIVE_WARC_DEFINED_PARAM.archive_fatal
        } else {
            if ver < 1200 as libc::c_uint || ver > 10000 as libc::c_uint {
                archive_set_error_safe!(&mut safe_a.archive as *mut archive,
                                  ARCHIVE_WARC_DEFINED_PARAM.archive_errno_misc,
                                  b"Unsupported record version: %u.%u\x00" as
                                      *const u8 as *const libc::c_char,
                                  ver.wrapping_div(10000 as libc::c_int as
                                                       libc::c_uint),
                                  ver.wrapping_rem(10000 as libc::c_int as
		 ...
		 * so I don't feel overly bad stopping the reader here */
            archive_set_error_safe!(&mut safe_a.archive as *mut archive,
                              ...
                              b"Bad content length\x00" as *const u8 as
                                  *const libc::c_char);
            return ARCHIVE_WARC_DEFINED_PARAM.archive_fatal
        }
        rtime =
		 ...
		 * so just barf here, fast and loud */
            archive_set_error_safe!(&mut safe_a.archive as *mut archive,
                              ARCHIVE_WARC_DEFINED_PARAM.einval,
                              b"Bad record time\x00" as *const u8 as
                                  *const libc::c_char);
            return ARCHIVE_WARC_DEFINED_PARAM.archive_fatal
        }
        ...
        if ver != safe_w.pver {
            ...
            unsafe{archive_string_sprintf(&mut safe_w.sver as *mut archive_string,
                                   b"WARC/%u.%u\x00" as *const u8 as
                                       *const libc::c_char,
                                   ver.wrapping_div(10000 as libc::c_int as
                                                        libc::c_uint),
                                   ver.wrapping_rem(10000 as libc::c_int as
        ...
        ftyp =
            _warc_rdtyp(buf,
                        unsafe{eoh.offset_from(buf) as libc::c_long as
                            size_t}) as warc_type_t;
        /* and let future calls know about the content */
        safe_w.cntlen =
            cntlen as size_t; /* Avoid compiling error on some platform. */
		 ...
		 * guaranteed to have one */
                fnam =
                    _warc_rduri(buf,
                                unsafe {eoh.offset_from(buf) as libc::c_long
                                    as size_t});
                /* check the last character in the URI to avoid creating
		 * directory endpoints as files, see Todo above */
                if unsafe{fnam.len == 0 as libc::c_int as libc::c_ulong ||
                       *fnam.str_0.offset(fnam.len.wrapping_sub(1 as
                                                                    libc::c_int
                                                                    as
                                                                    libc::c_ulong)
		 ...
		 * malloc()+free() roundtrip */
                    if fnam.len.wrapping_add(1 as libc::c_uint as
                                                 libc::c_ulong) >
                           safe_w.pool.len {
                        safe_w.pool.len =
                            fnam.len.wrapping_add(64 as libc::c_uint as
                                                      libc::c_ulong).wrapping_div(64
                                                                                      as
                                                                                      libc::c_uint
                                                                                      as
                                                                                      libc::c_ulong).wrapping_mul(64
                                                                                                                      as
                                                                                                                      libc::c_uint
                                                                                                                      as
                                                                                                                      libc::c_ulong);
                        safe_w.pool.str_0 =
                            realloc_safe(safe_w.pool.str_0 as *mut libc::c_void,
                                    safe_w.pool.len) as *mut libc::c_char
                    }
                    memcpy_safe(safe_w.pool.str_0 as *mut libc::c_void,
                           fnam.str_0 as *const libc::c_void, fnam.len);
                    unsafe {*safe_w.pool.str_0.offset(fnam.len as isize) =
        ...
        __archive_read_consume_safe(a,
                               unsafe{eoh.offset_from(buf) as libc::c_long});
        match ftyp as libc::c_uint {
            WT_RSRC| WT_RSP => {
                if fnam.len > 0 as libc::c_uint as libc::c_ulong { break ; }
            }
            WT_NONE | WT_INFO |WT_META |WT_REQ | WT_RVIS | WT_CONV | WT_CONT | LAST_WT | _ => { }
        }
        /* FALLTHROUGH */
 ...
 extern "C" fn _warc_read(mut a: *mut archive_read,
                                mut buf: *mut *const libc::c_void,
                                mut bsz: *mut size_t, mut off: *mut int64_t)
 -> libc::c_int {
    let mut w: *mut warc_s =unsafe{ (*(*a).format).data as *mut warc_s};
    ...
    if !(safe_w.cntoff >= safe_w.cntlen) {
        ...
        rab =
            __archive_read_ahead_safe(a, 1 as libc::c_uint as size_t, &mut nrd) as
                *const libc::c_char;
        if nrd < 0 as libc::c_int as libc::c_long {
            ...
            *safe_buf = rab as *const libc::c_void;
            safe_w.cntoff =
                (safe_w.cntoff as
                     libc::c_ulong).wrapping_add(nrd as libc::c_ulong) as
                    size_t as size_t;
    ...
    *safe_off =
        safe_w.cntoff.wrapping_add(4 as libc::c_uint as libc::c_ulong) as
            int64_t;
    safe_w.unconsumed = 0 as libc::c_uint as size_t;
    return ARCHIVE_WARC_DEFINED_PARAM.archive_eof;
}
 extern "C" fn _warc_skip(mut a: *mut archive_read) -> libc::c_int {
    ...
    let mut safe_w=unsafe{&mut *w};
    __archive_read_consume_safe(a,
                           safe_w.cntlen.wrapping_add(4 as libc::c_uint as
                                                        libc::c_ulong) as
                               int64_t);
...
/* private routines */
 extern "C" fn deconst(mut c: *const libc::c_void)
 -> *mut libc::c_void {
    return c as uintptr_t as *mut libc::c_void;
}
 extern "C" fn xmemmem(mut hay: *const libc::c_char, haysize: size_t,
                             mut needle: *const libc::c_char,
                             needlesize: size_t) -> *mut libc::c_char {
    ...
    if needlesize == 0 as libc::c_ulong {
        ...
        hay =
            memchr_safe(hay as *const libc::c_void, unsafe{*needle as libc::c_int},
                   haysize) as *const libc::c_char;
        if hay.is_null() {
            /* trivial */
    ...
    while hp < eoh && np < eon {
        ...
        eqp &=
            unsafe {(*hp as libc::c_int == *np as libc::c_int) as libc::c_int as
                libc::c_uint};
        unsafe {
        hp = hp.offset(1);
        np = np.offset(1)}
    ...
    while hp < eoh {
        ...
        if hsum == nsum &&
               memcmp_safe(cand as *const libc::c_void,
                      needle as *const libc::c_void,
                      needlesize.wrapping_sub(1 as libc::c_uint as
                                                  libc::c_ulong)) ==
                   0 as libc::c_int {
 ...
 extern "C" fn strtoi_lim(mut str: *const libc::c_char,
                                mut ep: *mut *const libc::c_char,
                                mut llim: libc::c_int, mut ulim: libc::c_int)
 -> libc::c_int {
    ...
    sp = str;
    rulim = (if ulim > 10 as libc::c_int { ulim } else { 10 as libc::c_int });
    while unsafe{res * 10 as libc::c_int <= ulim && rulim != 0 &&
    ...
    if sp == str {
        res = -(1 as libc::c_int)
    } else if res < llim || res > ulim { res = -(2 as libc::c_int) }
    unsafe {*ep = sp};
    return res;
}
 ...
 extern "C" fn xstrpisotime(mut s: *const libc::c_char,
 ...
 -> time_t {
    ...
    memset_safe(&mut tm as *mut tm as *mut libc::c_void, 0 as libc::c_int,
    ...
    /* read year */
    tm.tm_year =
        strtoi_lim(s, &mut s, 1583 as libc::c_int, 4095 as libc::c_int);
    if !(tm.tm_year < 0 as libc::c_int ||
        ...
        /* read month */
        tm.tm_mon =
            strtoi_lim(s, &mut s, 1 as libc::c_int, 12 as libc::c_int);
        if !(tm.tm_mon < 0 as libc::c_int ||
            ...
            /* read day-of-month */
            tm.tm_mday =
                strtoi_lim(s, &mut s, 1 as libc::c_int, 31 as libc::c_int);
            if !(tm.tm_mday < 0 as libc::c_int ||
                ...
                /* read hour */
                tm.tm_hour =
                    strtoi_lim(s, &mut s, 0 as libc::c_int,
                               23 as libc::c_int);
...
/* private routines */
extern "C" fn _warc_rdver(mut buf: *const libc::c_char,
                                 mut bsz: size_t) -> libc::c_uint {
    static mut magic: [libc::c_char; 6] =
        unsafe {
            *::std::mem::transmute::<&[u8; 6],
                                     &[libc::c_char; 6]>(b"WARC/\x00")
        };
    ...
    if bsz < 12 as libc::c_int as libc::c_ulong ||
           memcmp_safe(buf as *const libc::c_void,
                  unsafe {magic.as_ptr() as *const libc::c_void},
                  (::std::mem::size_of::<[libc::c_char; 6]>() as
                       libc::c_ulong).wrapping_sub(1 as libc::c_uint as
                                                       libc::c_ulong)) !=
               0 as libc::c_int {
    ...
    buf =
        unsafe {buf.offset((::std::mem::size_of::<[libc::c_char; 6]>() as
                        libc::c_ulong).wrapping_sub(1 as libc::c_uint as
                                                        libc::c_ulong) as
                       isize)};
    if unsafe{*(*__ctype_b_loc()).offset(*buf.offset(0 as libc::c_uint as isize) as
                                      libc::c_uchar as libc::c_int as isize)
           as libc::c_int &
           _ISdigit as libc::c_int as libc::c_ushort as libc::c_int != 0 &&
           *buf.offset(1 as libc::c_uint as isize) as libc::c_int ==
               '.' as i32 &&
           *(*__ctype_b_loc()).offset(*buf.offset(2 as libc::c_uint as isize)
                                          as libc::c_uchar as libc::c_int as
                                          isize) as libc::c_int &
               _ISdigit as libc::c_int as libc::c_ushort as libc::c_int != 0} {
        /* we support a maximum of 2 digits in the minor version */
        if unsafe{*(*__ctype_b_loc()).offset(*buf.offset(3 as libc::c_uint as isize)
                                          as libc::c_uchar as libc::c_int as
                                          isize) as libc::c_int &
               _ISdigit as libc::c_int as libc::c_ushort as libc::c_int != 0 }{
        ...
        ver =
            unsafe {((*buf.offset(0 as libc::c_uint as isize) as libc::c_int -
                  '0' as i32) as
                 libc::c_uint).wrapping_mul(10000 as libc::c_uint)};
        /* set up minor version */
        if end == 1 as libc::c_uint {
            ver =
                ver.wrapping_add(((unsafe{*buf.offset(2 as libc::c_uint as isize) as
                                       libc::c_int - '0' as i32}) as
                                      libc::c_uint).wrapping_mul(1000 as
                                                                     libc::c_uint));
            ver =
                ver.wrapping_add(((unsafe{*buf.offset(3 as libc::c_uint as isize) as
                                       libc::c_int - '0' as i32}) as
                                      libc::c_uint).wrapping_mul(100 as
                                                                     libc::c_uint))
        } else {
            ver =
                ver.wrapping_add(((unsafe{*buf.offset(2 as libc::c_uint as isize) as
        ...
        if ver >= 1200 as libc::c_uint {
            if memcmp_safe(c as *const libc::c_void,
                      b"\r\n\x00" as *const u8 as *const libc::c_char as
                          *const libc::c_void,
                      2 as libc::c_uint as libc::c_ulong) != 0 as libc::c_int
               {
                ver = 0 as libc::c_uint
            }
        } else if unsafe{*c as libc::c_int != ' ' as i32 &&
...
extern "C" fn _warc_rdtyp(mut buf: *const libc::c_char,
                                 mut bsz: size_t) -> libc::c_uint {
    ...
    val =
        xmemmem(buf, bsz, unsafe{_key.as_ptr()},
    ...
    val =
        unsafe {val.offset((::std::mem::size_of::<[libc::c_char; 13]>() as
                        libc::c_ulong).wrapping_sub(1 as libc::c_uint as
                                                        libc::c_ulong) as
                       isize)};
    eol =
        _warc_find_eol(val,
                       unsafe{buf.offset(bsz as isize).offset_from(val) as
                           libc::c_long as size_t});
    if eol.is_null() {
        /* no end of line */
        return WT_NONE as libc::c_int as libc::c_uint
    ...
    /* overread whitespace */
    while unsafe{val < eol &&
              (*val as libc::c_int == ' ' as i32 ||
                   *val as libc::c_int == '\t' as i32)} {
       unsafe { val = val.offset(1)}
    }
    if unsafe{val.offset(8 as libc::c_uint as isize)} == eol {
        if memcmp_safe(val as *const libc::c_void,
                  b"resource\x00" as *const u8 as *const libc::c_char as
                      *const libc::c_void, 8 as libc::c_uint as libc::c_ulong)
               == 0 as libc::c_int {
            return WT_RSRC as libc::c_int as libc::c_uint
        } else {
            if memcmp_safe(val as *const libc::c_void,
                      b"response\x00" as *const u8 as *const libc::c_char as
                          *const libc::c_void,
                      8 as libc::c_uint as libc::c_ulong) == 0 as libc::c_int
               {
 ...
 extern "C" fn _warc_rduri(mut buf: *const libc::c_char,
                                 mut bsz: size_t) -> warc_string_t {
    static mut _key: [libc::c_char; 19] =
        unsafe {
    ...
    let mut res: warc_string_t =
        {
            let mut init =
                warc_string_t{len: 0 as libc::c_uint as size_t,
                              str_0: 0 as *const libc::c_char,};
            init
        };
    val =
        xmemmem(buf, bsz, unsafe{_key.as_ptr()},
                (::std::mem::size_of::<[libc::c_char; 19]>() as
                     libc::c_ulong).wrapping_sub(1 as libc::c_uint as
                                                     libc::c_ulong));
    if val.is_null() {
        /* no bother */
    ...
    /* overread whitespace */
    val =
        unsafe {val.offset((::std::mem::size_of::<[libc::c_char; 19]>() as
                        libc::c_ulong).wrapping_sub(1 as libc::c_uint as
                                                        libc::c_ulong) as
                       isize)};
    eol =
    ...
    uri =
        xmemmem(val, unsafe{eol.offset_from(val) as libc::c_long as size_t},
    ...
    p = val;
    while p < eol {
        if unsafe{*(*__ctype_b_loc()).offset(*p as libc::c_uchar as libc::c_int as
                                          isize) as libc::c_int &
    ...
    if uri < unsafe{val.offset(3 as libc::c_uint as isize)} { return res }
    ...
    if !(memcmp_safe(val as *const libc::c_void,
                b"file\x00" as *const u8 as *const libc::c_char as
                    *const libc::c_void, 4 as libc::c_uint as libc::c_ulong)
             == 0 as libc::c_int) {
        if memcmp_safe(val as *const libc::c_void,
                  b"http\x00" as *const u8 as *const libc::c_char as
                      *const libc::c_void, 4 as libc::c_uint as libc::c_ulong)
               == 0 as libc::c_int ||
               memcmp_safe(val as *const libc::c_void,
                      b"ftp\x00" as *const u8 as *const libc::c_char as
                          *const libc::c_void,
                      3 as libc::c_uint as libc::c_ulong) == 0 as libc::c_int
           {
 ...
 extern "C" fn _warc_rdlen(mut buf: *const libc::c_char,
                                 mut bsz: size_t) -> ssize_t {
    static mut _key: [libc::c_char; 18] =
        unsafe {
    ...
    val =
        unsafe {val.offset((::std::mem::size_of::<[libc::c_char; 18]>() as
                        libc::c_ulong).wrapping_sub(1 as libc::c_uint as
                                                        libc::c_ulong) as
                       isize)};
    eol =
    ...
    if unsafe{*(*__ctype_b_loc()).offset(*val as libc::c_uchar as libc::c_int as
                                      isize) as libc::c_int &
           _ISdigit as libc::c_int as libc::c_ushort as libc::c_int == 0} {
        return -(1 as libc::c_int) as ssize_t
    }
    ...
    if unsafe{*__errno_location_safe() != 0 as libc::c_int }||
           on != eol as *mut libc::c_char {
        /* line must end here */
        return -(1 as libc::c_int) as ssize_t
    }
    return len as size_t as ssize_t;
}
 extern "C" fn _warc_rdrtm(mut buf: *const libc::c_char,
                                 mut bsz: size_t) -> time_t {
    static mut _key: [libc::c_char; 13] =
        unsafe {
            *::std::mem::transmute::<&[u8; 13],
                                     &[libc::c_char; 13]>(b"\r\nWARC-Date:\x00")
 ...
 extern "C" fn _warc_rdmtm(mut buf: *const libc::c_char,
                                 mut bsz: size_t) -> time_t {
    static mut _key: [libc::c_char; 17] =
        unsafe {
    ...
    val =
        unsafe{val.offset((::std::mem::size_of::<[libc::c_char; 17]>() as
                        libc::c_ulong).wrapping_sub(1 as libc::c_uint as
                                                        libc::c_ulong) as
                       isize)};
    eol =
 ...
 extern "C" fn _warc_find_eoh(mut buf: *const libc::c_char,
                                    mut bsz: size_t) -> *const libc::c_char {
    static mut _marker: [libc::c_char; 5] =
        unsafe {
            *::std::mem::transmute::<&[u8; 5],
                                     &[libc::c_char; 5]>(b"\r\n\r\n\x00")
        };
    let mut hit: *const libc::c_char =
        xmemmem(buf, bsz, unsafe{_marker.as_ptr()},
                (::std::mem::size_of::<[libc::c_char; 5]>() as
                     libc::c_ulong).wrapping_sub(1 as libc::c_uint as
                                                     libc::c_ulong));
    if !hit.is_null() {
        hit =
            unsafe {hit.offset((::std::mem::size_of::<[libc::c_char; 5]>() as
                            libc::c_ulong).wrapping_sub(1 as libc::c_uint as
                                                            libc::c_ulong) as
                           isize)}
    }
 ...
 extern "C" fn _warc_find_eol(mut buf: *const libc::c_char,
                                    mut bsz: size_t) -> *const libc::c_char {
